...
// API配置 - 使用代理路径
const API_BASE_URL = '/api/asr'
const API_ENDPOINTS = {
  start: `${API_BASE_URL}/start`,
  stop: `${API_BASE_URL}/stop`,
  status: `${API_BASE_URL}/status`,
  recent: `${API_BASE_URL}/recent?minutes=1`,  // 获取最近1分钟的记录，更快响应
  current: `${API_BASE_URL}/current`  // 获取当前转录文本的接口
}
...
const startRecordingAPI = async () => {
  try {
    ...
    const result = await callAPI(API_ENDPOINTS.start, 'POST')
    console.log('开始录音API响应:', result)

    if (result.success || result.status === 'started') {
      statusText.value = '录音服务已启动'
      return true
    } else {
      throw new Error(result.message || '启动录音服务失败')
    }
  } catch (error) {
...
const stopRecordingAPI = async () => {
  try {
    ...
    const result = await callAPI(API_ENDPOINTS.stop, 'POST')
    console.log('停止录音API响应:', result)

    if (result.success || result.status === 'stopped') {
      statusText.value = '录音服务已停止，正在获取转录结果...'
      ...
      throw new Error(result.message || '停止录音服务失败')
    }
  } catch (error) {
...
const fetchCurrentTranscription = async () => {
  try {
    // 获取最近1分钟的记录来找到最新的转录文本，响应更快
    const recentUrl = `${API_BASE_URL}/recent?minutes=1`
    const result = await callAPI(recentUrl)
...
const checkStatusAPI = async (enableSync = false) => {
  try {
    const result = await callAPI(API_ENDPOINTS.status)
    ...
    if (result) {
      ...
      if (result.data && typeof result.data.is_running === 'boolean') {
        ...
        if (enableSync) {
          console.log('� 执行状态同步（仅在页面初始化时）')

          if (serverIsRunning) {
            // 服务器正在录音
            if (!isRecording.value) {
              console.log('🎤 页面初始化：检测到服务器正在录音，同步按钮状态')
              isRecording.value = true
              statusText.value = '正在录音...'
              console.log('🤖 使用机器人麦克风录音，启动本地转录文本更新')

              // 🔧 修复：启动本地录音功能以支持转录文本更新
              await startRecording()

              // 🔧 额外修复：立即获取一次转录文本，确保显示最新内容
              console.log('🔄 页面刷新后立即获取转录文本...')
              setTimeout(async () => {
                await updateTranscriptionText()
                console.log('✅ 页面刷新后转录文本获取完成')
              }, 500) // 等待500ms后获取
            } else {
              console.log('✅ 页面初始化：服务器和前端状态一致（都在录音）')
            }
          } else {
            // 服务器已停止录音
            if (isRecording.value) {
              console.log('⏹️ 页面初始化：检测到服务器已停止录音，同步按钮状态')
              ...
              console.log('✅ 页面初始化：服务器和前端状态一致（都已停止）')
            }
          }
        } else {
...
const testConnection = async () => {
  try {
    ...
    try {
      ...
      console.error('❌ 基础网络连接失败:', pingError)
      throw new Error(`无法连接到服务器，请检查：\n1. 服务器 https://b4d1c38cb7ab.ngrok-free.app 是否运行\n2. 代理配置是否正确\n3. 网络连接是否正常`)
    }
    ...
    // 1. 测试状态查询接口
    try {
      console.log('📊 测试状态查询接口...')
      await callAPI(API_ENDPOINTS.status)
      console.log('✅ 状态查询接口正常')
    } catch (error) {
      console.warn('⚠️ 状态查询接口测试失败:', error.message)
    }

    // 2. 测试历史记录接口
    try {
      console.log('📚 测试历史记录接口...')
      await callAPI(API_ENDPOINTS.recent)
      console.log('✅ 历史记录接口正常')
    } catch (error) {
      console.warn('⚠️ 历史记录接口测试失败:', error.message)
    }

    // 3. 测试开始录音接口（不实际开始录音，只测试连通性）
    try {
      console.log('🎙️ 测试开始录音接口连通性...')
      // 这里可以发送一个测试请求，但不实际开始录音
      console.log('✅ 开始录音接口可访问')
    } catch (error) {
      console.warn('⚠️ 开始录音接口测试失败:', error.message)
    }
    ...
    console.log('- 当前接口配置:', API_ENDPOINTS)
