...
const diagnoseVideoStream = async () => {
  ...
  try {
    // 1. 测试基础连接
    console.log('📡 测试基础连接...')
    const headResponse = await fetch(CAMERA_API_URL, { method: 'HEAD' })
    console.log(`📡 HEAD请求结果: ${headResponse.status} ${headResponse.statusText}`)

    // 2. 测试GET请求
    console.log('📥 测试GET请求...')
    const getResponse = await fetch(CAMERA_API_URL, { method: 'GET' })
...
.btn-small {
  padding: clamp(0.4rem, 1.2vw, 0.8rem) clamp(0.6rem, 1.5vw, 1.2rem);
...
@media (min-width: 1025px) {
  ...
  .leg-right-section {
    flex: 1.2;
    max-width: 55%;
    min-width: clamp(350px, 40vw, 600px);
  }
}
</style>
